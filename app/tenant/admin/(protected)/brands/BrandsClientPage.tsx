...
export default function BrandsClientPage({ initialBrands = [], tenant }: BrandsPageProps) {
  ...
  const handleDelete = async (brandId: string, brandName: string) => {
    if (!confirm(`ต้องการลบแบรนด์ "${brandName}" หรือไม่?\n\nการดำเนินการนี้ไม่สามารถย้อนกลับได้`)) {
      return
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(result.message || 'เกิดข้อผิดพลาดในการลบแบรนด์')
      }
    } catch (error) {
      ...
      {filteredBrands.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBrands.map((brand) => (
            <div key={brand.id} className="card hover:shadow-lg transition-shadow relative">
              {deleting === brand.id && (
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center space-x-4">
                  {/* Brand Logo */}
                  <div className="w-16 h-16 rounded-lg bg-secondary-100 flex items-center justify-center overflow-hidden">
                    {brand.logo ? (
                      <Image
                        src={brand.logo}
                        alt={brand.name}
